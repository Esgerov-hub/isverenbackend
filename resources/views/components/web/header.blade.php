            ...
            <div class="container">
                <div class="navbar-flex d-flex align-items-center justify-content-between w-100 pb-3 pt-3">
                        ...
                        <ul class="nav navbar-nav" id="responsive-menu">
                            @if(!empty(auth()->guard('web')->user()->userRole->role) && (auth()->guard('web')->user()->userRole->role->slug == 'users' || auth()->guard('web')->user()->userRole->role->slug == 'admin'))
                                <li class="register" style="display: none;">
                                    <a href="{{ route('web.user.cv') }}" >Cv yarat</a>
                                </li>
                            @elseif(!empty(auth()->guard('web')->user()->userRole->role) && (auth()->guard('web')->user()->userRole->role->slug == 'company' || auth()->guard('web')->user()->userRole->role->slug == 'admin'))
                                <li class="register" style="display: none;">
                                    <a href="{{ route('web.user.jobs.list') }}" >Vakansiyalarım</a>
                                </li>
                            @else
                                <li class="register" style="display: none;">
                                    <a href="" data-bs-toggle="modal" data-bs-target="#exampleModal">Qeydiyyat</a>
                                </li>
                                <li class="access" style="display: none;">
                                    <a href="" data-bs-toggle="modal" data-bs-target="#exampleModal1">Giriş et</a>
                                </li>
                            @endif
                            <li><a href="{{ route('web.vacancy') }}">@lang('web.jobs')</a></li>

                                <li><a href="{{ route('web.professions') }}">İxtisaslar</a></li>

                                <li><a href="{{ route('web.companies') }}">@lang('web.companies')</a></li>
                            <li><a href="{{ route('web.blogs') }}">@lang('web.blogs')</a></li>
                                <li><a href="{{ route('web.cv') }}">İş axtaranlar</a></li>
                    ...
                    <div class="register-login d-flex align-items-center">
                        @if(!empty(auth()->guard('web')->user()->userRole->role) && (auth()->guard('web')->user()->userRole->role->slug == 'company' || auth()->guard('web')->user()->userRole->role->slug == 'admin'))
                        ...
                        @elseif(!empty(auth()->guard('web')->user()->userRole->role) && (auth()->guard('web')->user()->userRole->role->slug == 'users' || auth()->guard('web')->user()->userRole->role->slug == 'admin'))
                            <a  href="{{ route('web.user.cv') }}" class="job-top-item d-flex align-items-center border-b">
                                <figure class="me-3 mb-0">
                                    <img alt="" src="{{ asset('user/user.png') }}" style="width: 30px; height: 31px; border-radius: 50%; !important;">
                                </figure>
                                <div class="job-info-creator">
